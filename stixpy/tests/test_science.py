...
def test_sciencedata_get_data():
    ...
    norm = l1.data["timedel"].reshape(5, 1, 1, 1) * l1.dE
    rate = tot / norm
    error = np.sqrt(tot * u.ct + l1.data["counts_comp_err"][..., 0:12, :].sum(axis=2, keepdims=True) ** 2) / norm
    ...
    rate = tot / norm
    error = (
        np.sqrt(tot * u.ct + l1.data["counts_comp_err"][:, 0:32, 0:12, :].sum(axis=(1, 2), keepdims=True) ** 2) / norm
    )
    ...
    norm = l1.data["timedel"].reshape(5, 1, 1, 1) * (l1.energies[30]["e_high"] - l1.energies[1]["e_low"])
    rate = tot / norm
    ...
    count, count_err, times, timedel, energies = l1.get_data(time_indices=[[0, 4]], energy_indices=[[1, 30]])
